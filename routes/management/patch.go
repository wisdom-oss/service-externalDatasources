...
func UpdateDataSourceRepresentation(w http.ResponseWriter, r *http.Request) {
	...
	if ok {
		...
		if err := json.Unmarshal([]byte(rawMetadataObject), &dataSource.Metadata); err != nil {
	...
	if ok {
		...
		if err := json.Unmarshal([]byte(rawAPIObject), &dataSource.API); err != nil {
			// now check if an error happened during the unmarshalling
			var unmarshalError *json.InvalidUnmarshalError
			var syntaxError *json.SyntaxError
			var typeError *json.UnmarshalTypeError
	...
	// to secure the following update queries, a transaction will be used
	...
	_, err = globals.SqlQueries.Exec(tx, "set-base-data",
		uuid, dataSource.Name, dataSource.Description)
	if err != nil {
	...
	_, err = globals.SqlQueries.Exec(tx, "set-metadata",
		uuid, metadata.Reference, metadata.DataOrigin,
		metadata.DistinctiveFeatures, metadata.UsageRights,
		metadata.UsageDuties, metadata.Entities, metadata.Expert,
		metadata.Documentation, metadata.UpdateRate, metadata.Languages,
		metadata.Billing, metadata.DataProvision, metadata.DerivedFrom,
		metadata.Recent, metadata.Validity, metadata.Duplicates,
		metadata.Errors, metadata.Precision, metadata.Reputation,
		metadata.Objectivity, metadata.KnownCaptureMethod,
		metadata.Density, metadata.Coverage,
		metadata.RepresentationalConsistency, metadata.LogicalConsistency,
